...
class Amf3Reader
{
	...
	private function readWithCode(marker:Int):Dynamic
	{
		...
		return switch (marker)
		{
			...
			default:
				throw "Not supported or wrong amf type-marker: " + marker;
	...
	private function readAmf3Date():Date
	{
		...
		// trace(" -- reading int");
		return flag & 1 == 0 ? Date.fromTime(input.readDouble()) : cast decodedObjects[flag >> 1];
	}
	...
	private function readAmf3Object():Dynamic
	{
		...
		else
		{
			...
			else
			{
				...
				{
					class_attributes.push(readAmf3String());
				} // Read class members
	...
	**/
	public static function deepCopy<T>(v:T):T
	{
		...
		else if (Std.isOfType(v, Array)) // array
		{
