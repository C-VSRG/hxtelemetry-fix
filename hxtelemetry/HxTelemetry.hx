...
class Timing
{
	...
	public static var DEFAULT_DESCRIPTORS:Array<ActivityDescriptor> = [{name: GC, description: "Garbage Collection", color: 0xdd5522}];
...
class HxTelemetry
{
	...
	static inline function timestamp_ms():Float
	{
		return _abs_t0_usec / 1000 + haxe.Timer.stamp() * 1000;
	};

	static inline function timestamp_us():Float
	{
		return _abs_t0_usec + haxe.Timer.stamp() * 1000000;
	};
	...
	public function new(config:Config = null)
	{
		...
		if (config.trace)
		{
			...
			haxe.Log.trace = function(v:Dynamic, ?infos):Void
			{
				...
				if (_writer != null)
					_writer.sendMessage({"name": ".trace", "value": (infos == null ? '' : infos.fileName + ":" + infos.lineNumber + ": ") + s});
				oldTrace(v, infos);
			}
		}
	...
	public function send_frame_delimiter():Void
	{
		...
		while (_restart.length > 0)
		{
			start_timing(_restart.pop());
		}
		disable_alloc_tracking(false);
	}
		...
		function try_connect()
		{
			...
			try
			{
				...
				write_object({
					"name": ".swf.name",
					"value": app_name,
					"hxt": switch_to_nonamf,
					"activity_descriptors": activity_descriptors
				});
				if (switch_to_nonamf)
				{
					amf_mode = false;
	...
	public inline static function dump_telemetry_frame(thread_num:Int, output:haxe.io.Output, write_object:Dynamic->Void):Void
	{
		#if (cpp && HXCPP_TELEMETRY)
